<template>
  <q-header elevated class="bg-primary text-white">
    <q-toolbar>
      ...
      <q-toolbar-title class="text-h6"> Painel do Funcionário </q-toolbar-title>
...
// Prop opcional já que não está sendo usada
defineProps<{
...
// Computed property segura para o nome do usuário
const userDisplayName = computed(() => {
  if (!authStore.user) return 'Funcionário';

  const user = authStore.user;

  if (user.fullName?.display) {
    return user.fullName.display;
  }
...
const unreadNotifications = computed(() => appStore.unreadNotifications || 0);
const notifications = computed(() => appStore.notifications || []);

function goToNotifications(): void {
  void router.push('/staff/notifications');
}

function goToProfile(): void {
  void router.push('/staff/profile');
}
